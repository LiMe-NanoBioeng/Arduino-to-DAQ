...
//by :K2 20260811 fix preT/curT float overflow: store millis() as unsigned long and take unsigned differences
...
int PID(){
  ...
  max_val=10;
  curT=millis();
  dt = (curT-preT)/1000.0; // unsigned subtraction wraps correctly across millis() rollover
...
//     Serial.println((float) millis());
  ...
  while (delayTime > (millis()-curT)/1000.0){
    // just a timer, unsigned subtraction wraps correctly across millis() rollover
    //Serial.println(val);
  }
  ...
  while ((delayTime+duration > (millis()-curT)/1000.0) && (val < threshold)){
    val = analogRead(vNumRead);// stop threshold
    //Serial.println(val);
  }
  if ( from_to == ':'){
