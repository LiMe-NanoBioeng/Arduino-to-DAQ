...
int val = 0;
int detect_pulse_in=11;
int detect_total=0;
int detect_flag=0;
int detect_ttl_out=0;
...
void setup() {
  ...
  pinMode(detect_pulse_in,INPUT);
  pinMode(detect_ttl_out,OUTPUT);
 ...
 starttime=millis();
  do {
    ...
    if (millis()-starttime > 10000){
      fmeter=false;
      //now we use pressure sensor 251204 :K2
      break;
      }
...
void loop() {
 ...
 float p_volt=analogRead(A8)*5.0/1023.0;
float pressure=250.0*(p_volt-2.5);
...
int PID(){
  ...
  if (is_first_cycle){
    ...
    float p_volt=analogRead(A0)*5.0/1023.0;
    float measurement=250.0*(p_volt-2.5);
  }
...
void pulseDetecter(){
  ...
  if(digitalRead(detect_pulse_in)==1 && detect_flag==0){
     Serial.print("flag: ");
 Serial.print(detect_flag);
 Serial.print(" read: ");
Serial.println(digitalRead(detect_pulse_in));
detect_total++;
    detect_flag=1;
  ...
  Serial.println(detect_total);
    for(int i =0; i<=0 ;i++){
      digitalWrite(detect_ttl_out,HIGH);
      delayMicroseconds(10);
      digitalWrite(detect_ttl_out,LOW);
      delayMicroseconds(100);
    }
  }else if(digitalRead(detect_pulse_in)==0 && detect_flag==1  ){
    detect_flag=0;
  }
}